...
function CircleMenu(target, data, selectedByKey, opt, func) {
	...
	this.init = function() {
		...
		if(data && data.length > 0) {
			...
			document.getElementById('circle-info').innerHTML = textForCircle[that.currentSelected]
			_.forEach(that.items, function(d, i){
				d.find('a').on('click', function(){
					that.select(i);
					document.getElementById('circle-info').innerHTML = textForCircle[i] || ''
					return false;
				});
			});
	...
	this.createItems = function() {
		...
		_.forEach(data, function(d, i){
			cm_items.append('<li id="item-'+i+'" class="cm-item"><a>'+d.label+'</a></li>');
